...
[![Infrastructure as Code](https://img.shields.io/badge/CloudFormation-Infrastructure%20as%20Code-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white)](cloudformation/seatrelay-serverless.yml)
...
The single-instance deployment is a deliberate choice, not a limitation we discovered late. Every component was written so it relocates rather than gets rewritten, and the managed architecture is defined as infrastructure-as-code — 515 lines of CloudFormation covering:

| Layer | Serving the demo today | Defined in CloudFormation |
|---|---|---|
...
| CloudFormation definition of the managed stack: Cognito, DynamoDB, Lambda, Step Functions, SNS, S3 | |
...
cloudformation/
  seatrelay-serverless.yml   managed stack: Cognito · DynamoDB · Lambda · Step Functions · SNS · S3
...
| Member | Owned |
|---|---|
| **Hirthik Balaji** | Resale protocol design, transfer state machine, REST API, e-Rupee settlement engine, React application, CloudFormation stack |
...
| **Anandhappriya** | Demo production and pitch — market research on cancellation policy and the IRCTC transfer precedent, the walkthrough video and deck, and manual QA across all three personas on the deployed instance |
